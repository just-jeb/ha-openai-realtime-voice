...
void VoiceAssistantWebSocket::websocket_cleanup_task_(void *arg) {
  ...
  ESP_LOGD(TAG, "[diag] cleanup finished in %ums", (unsigned) elapsed);
...
void VoiceAssistantWebSocket::loop() {
  ...
  // Phase triggers are mutually exclusive — only fire the latest one.
  // The WS handler clears other phase bits when setting a new phase, so normally
  // only one is set. The else-if chain is a safety net if a race sets multiple.
  if (triggers & TRIGGER_REPLYING) {
    this->replying_trigger_.trigger();
  } else if (triggers & TRIGGER_SEARCHING) {
    this->searching_trigger_.trigger();
  } else if (triggers & TRIGGER_LISTENING) {
...
void VoiceAssistantWebSocket::start() {
  ...
  ESP_LOGD(TAG, "[diag] start conn #%u heap=%u", (unsigned) this->connection_count_,
...
void VoiceAssistantWebSocket::send_audio_chunk_(const uint8_t *data, size_t len) {
  ...
  if (this->audio_chunks_sent_ <= 3) {
    ESP_LOGD(TAG, "[diag] send #%u ret=%d +%ums",
...
void VoiceAssistantWebSocket::handle_websocket_event_(esp_websocket_event_id_t event_id,
  ...
  if (event_id != WEBSOCKET_EVENT_DATA || event_data == nullptr ||
    ...
    ESP_LOGD(TAG, "[diag] WS event id=%d state=%d",
  ...
  switch (event_id) {
    ...
    case WEBSOCKET_EVENT_CONNECTED:
      ...
      ESP_LOGD(TAG, "[diag] connected conn #%u heap=%u",
    ...
    case WEBSOCKET_EVENT_DATA:
      if (!this->first_data_after_connect_) {
        ...
        ESP_LOGD(TAG, "[diag] first DATA event after connect: +%ums op=0x%02x",
      ...
      if (event_data->op_code == 0x02) {
        ...
        if (message.find("\"type\":\"ready\"") != std::string::npos ||
          ...
          if (message.find("\"phase\":\"thinking\"") != std::string::npos ||
            ...
            this->pending_triggers_ = (this->pending_triggers_ & ~PHASE_MASK) | TRIGGER_THINKING;
            ...
            this->searching_phase_active_ = false;
            this->last_speaker_audio_time_ = millis();
            this->pending_triggers_ = (this->pending_triggers_ & ~PHASE_MASK) | TRIGGER_REPLYING;
            ...
            this->searching_phase_active_ = false;
            this->pending_triggers_ = (this->pending_triggers_ & ~PHASE_MASK) | TRIGGER_LISTENING;
            ...
            this->last_speaker_audio_time_ = millis();
            this->pending_triggers_ = (this->pending_triggers_ & ~PHASE_MASK) | TRIGGER_SEARCHING;
    ...
    case WEBSOCKET_EVENT_ERROR:
      if (event_data != nullptr) {
        ...
        ESP_LOGD(TAG, "[diag] error conn #%u +%ums type=%d errno=%d chunks_sent=%u heap=%u",
                 (unsigned) this->connection_count_,
                 this->connect_millis_ ? (unsigned) (millis() - this->connect_millis_) : 0u,
                 event_data->error_handle.error_type,
                 event_data->error_handle.esp_transport_sock_errno,
